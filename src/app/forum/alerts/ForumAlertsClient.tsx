...
export default function ForumAlertsClient() {
  ...
  useEffect(() => {
    const handler = (msg: any) => {
      ...
      if (msg && typeof msg === 'object' && typeof msg.channel === 'string' && msg.channel.toLowerCase() !== 'alerts') {
        return;
      }
  ...
  if ((own || wasAtBottom) && chatRef.current) setTimeout(() => { ensureScrollToBottom(); }, 0);
    };
  socket.on("message:alerts", handler);
  return () => { socket.off("message:alerts", handler); };
  }, [username, isAdmin]);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoadingInit(true);
        const res = await fetch('/api/messages/alerts?limit=15', { credentials: 'include' });
        if (res.ok) {
          const data = await res.json();
  ...
  const handleScroll = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/messages/alerts?limit=15&before=${before}`, { credentials: 'include' });
      if (!res.ok) return;
      const data = await res.json();
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin) return; // non-admins cannot send
  ...
  await axios.post('/api/socket-proxy/message', { message: finalMessage, sender: uname, channel: 'alerts', admin: isAdmin });
      try {
        await fetch('/api/messages/alerts', {
          method: 'POST',
          credentials: 'include',
