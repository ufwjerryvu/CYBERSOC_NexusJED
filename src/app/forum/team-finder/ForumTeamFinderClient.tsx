...
// NOTE: Channel isolation will be wired in a later step; for now this mirrors General behavior.
export default function ForumTeamFinderClient() {
  ...
  useEffect(() => {
    const handler = (msg: any) => {
      ...
      if (msg && typeof msg === 'object' && typeof msg.channel === 'string' && msg.channel.toLowerCase() !== 'team-finder') {
        return;
      }
  ...
  socket.on("message:team-finder", handler);
  return () => { socket.off("message:team-finder", handler); };
  }, [username, isAdmin]);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoadingInit(true);
  const res = await fetch('/api/messages/team-finder?limit=15', { credentials: 'include' });
        if (res.ok) {
          const data = await res.json();
  ...
  const res = await fetch(`/api/messages/team-finder?limit=15&before=${before}`, { credentials: 'include' });
      if (!res.ok) return;
      const data = await res.json();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ensureScrollToBottom();
  // route through same-origin Next.js proxy to avoid CORS
  await axios.post('/api/socket-proxy/message', { message: finalMessage, sender: uname, channel: 'team-finder', admin: isAdmin });
      try {
        await fetch('/api/messages/team-finder', {
          method: 'POST',
          credentials: 'include',
